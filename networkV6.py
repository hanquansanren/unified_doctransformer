...
class DilatedResnet(nn.Module):
	def __init__(self, n_classes, num_filter, BatchNorm, target_control_points, in_channels=3):
		...
		self.out_regress = nn.Sequential(
			nn.Conv2d(self.num_filter * map_num[2], self.num_filter * map_num[1], kernel_size=3, stride=2, padding=1),
			# channel: input 128, output 32
			# size: 尺度16*16
			BatchNorm(self.num_filter * map_num[1]),
			nn.PReLU(),

			nn.Conv2d(self.num_filter * map_num[1], self.num_filter * map_num[0], kernel_size=3, stride=2, padding=1),
			# channel: input 128, output 32, 尺度8*8
			BatchNorm(self.num_filter * map_num[0]),
			nn.PReLU(),
