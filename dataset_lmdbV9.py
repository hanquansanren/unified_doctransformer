...
class my_unified_dataset(data.Dataset):
	def __init__(self, root, mode='train'):
		...
		if self.mode == 'test':
			...
			for self.idx, (key, value) in enumerate(self.txn.cursor()):
				...
				key_set.append(key)
				if ((self.idx+1)%4)==0:
					# self.image_set[self.mode][key.decode().split("_")[2]]=key_set   # for onelmdb
					self.image_set[self.mode][key.decode().split("_")[3]]=key_set # for biglmdb, fast train
					key_set = []
			print("finished data")
		else:
	...
	def __getitem__(self, item):
		if self.mode == 'test':
			...
			# key=item: '0' ~ '850'
			# value: [b'0_0_0018_18_d1', b'0_0_0018_18_d2', b'0_0_0018_18_di', b'0_0_0018_18_w1']
			# d1, lbl1, d2, lbl2, di, w1 = self.read_img_lmdb(self.env, im_set_key_list)
			d1, lbl1, d2, lbl2= self.read_img_lmdb(self.env, im_set_key_list)
			...
			# # '''visualization edge point for resized synthesized image''
			...
			# # '''visualization resized synthesized image and its control point'''
