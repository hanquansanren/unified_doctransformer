...
class createThinPlateSplineShapeTransformer(nn.Module):
    ...
    def forward(self, batch_I, batch_F, shap_new=None):
        ...
        shap_new=flat_shap:(list) h,w,[1521, 1137] 用于生成regular的参考点
        '''
        # # 这一步就完成了320*320形式的 backward mapping构建
        ...
        # 这一步就完成了h*w形式的 backward mapping构建
        build_P_prime = self.estimateTransformation.build_P_prime(batch_F)  
        build_P_prime_reshape = build_P_prime.reshape([build_P_prime.size(0), self.I_r_size[0], self.I_r_size[1], 2])  # build_P_prime.size(0) == mini-batch size,
        # output: [1, 320, 320, 2]
        ...
        if shap_new is None:
            ...
            # map = F.interpolate(build_P_prime_reshape, shap_new,  mode='bilinear', align_corners=True) # [1, 2, 320, 320]-->[1, 2, 1521, 1137] # 这里的插值函数仅支持NCHW的形式，故而需要手动转换
            # map = map.transpose(1, 2).transpose(2, 3) # [1, 2, 1521, 1137]-->[1, 1521, 1137, 2] 至此，获得了backward mapping
            map = build_P_prime_reshape
