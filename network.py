...
class DilatedResnetForFlatByFiducialPointsS2(nn.Module):
	...
	def __init__(self, n_classes, num_filter, BatchNorm, in_channels=3):
		...
		self.n_classes = n_classes # 标准控制点的长宽间距
	...
	def forward(self, x, is_softmax):
		resnet_head = self.resnet_head(x) # 图示的第一层
		resnet_down = self.resnet_down(resnet_head) # 图示的中四层
		...
		bridge = self.bridge_concate(bridge_concate)# 图示最后一层
