...
2022/9/30
Weiguang Zhang
V9 means better initial+polar doc
'''
...
class model_handlebar(nn.Module):
	...
	def __init__(self, n_classes, num_filter, architecture, BatchNorm='GN', in_channels=3):
        ...
        #     np.linspace(0, 992, 31),
        #     np.linspace(0, 992, 31),)))


		# 这里实现了类DilatedResnet的实例化
		self.dilated_unet = architecture(self.n_classes, self.num_filter, BatchNorm, target_control_points=None, in_channels=self.in_channels)
...
class DilatedResnet(nn.Module):
	def __init__(self, n_classes, num_filter, BatchNorm, target_control_points=None, in_channels=3):
		...
		self.out_regress = nn.Sequential(
			...
			# nn.Conv2d(self.num_filter * map_num[1], self.num_filter * map_num[0], kernel_size=3, stride=2, padding=1),
			# # channel: input 64, output 32, 尺度8*8
			# BatchNorm(self.num_filter * map_num[0]),
			# nn.PReLU(),


			nn.Conv2d(self.num_filter * map_num[0], self.n_classes, kernel_size=3, stride=1, padding=1)
			# channel: input 32, output 2
			# feature size: 31*31
		)
		...
		# bias = self.target_control_points.transpose(1,0) # (2,961) ((X,Y),(X,Y),(X,Y),...) # 列序优先
	...
	def forward(self, images1, images2=None, w_im=None):
		...
		# out_regress1 = out_regress1.view(-1, 2, 961, 1)
